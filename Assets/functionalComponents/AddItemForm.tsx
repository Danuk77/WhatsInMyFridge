...
export function AddItemForm(props: AddItemFormProps): React.JSX.Element {
    ...
    // refs
    // used to call a function inside the date boxes so that they parse their values when the submit button is pressed.
    const dateBox0 = useRef<DateSelectorBoxRef | null>(null);
    const dateBox1 = useRef<DateSelectorBoxRef | null>(null);
    ...
    function handleSubmit() {
        ...
        // parse data in these boxes now
        // we have to get the output from here and not from the Date state variable
        // ... because if this parse results in the state variable changing then it won't have reloaded during the run of this function
        const expirationDateParsed = dateBox0.current?.handleTextDefocus();
        const dateAddedParsed = dateBox1.current?.handleTextDefocus();
        ...
        props.onSubmit({
            ...
            expirationDate: moment(expirationDateParsed).format("YYYY-MM-DD"),
            startDate: moment(dateAddedParsed).format("YYYY-MM-DD"),
